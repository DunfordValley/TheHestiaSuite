...
**OAuth callback is a browser redirect, not an XHR call.** The `/api/gmail/callback` route is reached via a browser navigation from Google, not an Axios call. The correct redirect URI depends on how the app is running — all registered URIs must be added to Google Cloud Console:

| Environment | `GOOGLE_REDIRECT_URI` |
|---|---|
| Local Docker (`docker compose up`) | `http://localhost/api/gmail/callback` — nginx on port 80 proxies `/api/` to the backend |
| Local Vite dev server (`npm run dev`) | `http://localhost:5173/api/gmail/callback` — Vite's dev proxy forwards the callback |
| Production (Cloudflare tunnel) | `https://crm.yourcompany.com/api/gmail/callback` |
...
**Tracking pixel must be publicly reachable.** Gmail fetches tracking pixels through its own proxy servers from the public internet. The `PUBLIC_URL` env var must be the Cloudflare tunnel URL. For local Docker testing, set `PUBLIC_URL=http://localhost` — the send flow works, but open tracking will not register because Gmail cannot fetch the pixel from a local address. `COALESCE(opened_at, NOW())` ensures the first-open timestamp is never overwritten by subsequent prefetches.
...
# Set to whichever matches your running environment:
#   Local Docker:  http://localhost/api/gmail/callback
#   Local Vite:    http://localhost:5173/api/gmail/callback
...
# Production: https://crm.yourcompany.com  (open tracking works)
# Local:      http://localhost              (send works; open tracking does not)
PUBLIC_URL=https://crm.yourcompany.com

# Optional: sync interval in milliseconds (default 300000 = 5 minutes)
# GMAIL_SYNC_INTERVAL_MS=300000
```
...
GMAIL_SYNC_INTERVAL_MS: ${GMAIL_SYNC_INTERVAL_MS:-300000}
```
...
**Dockerfile note:** The backend Dockerfile builder stage uses `npm install` (not `npm ci`) so that Docker can resolve newly added dependencies even when `package-lock.json` has not yet been regenerated locally. The production stage copies the lockfile generated by the builder and uses `npm ci --omit=dev` for a clean, reproducible production install.
...
| Phase | Work | Status |
|---|---|---|
| **A — Infrastructure** | `002_gmail.sql` migration; `googleapis` install; `gmail.ts` service; `/api/gmail/status` route; mount in `index.ts` | ✅ Done |
| **B — OAuth Flow** | Google Cloud Console setup; `/auth` + `/callback` + `/disconnect` routes; end-to-end token storage | ✅ Done |
| **C — Settings UI** | `SettingsPage.tsx`; `/settings` route; nav item; connect/disconnect flow | ✅ Done |
| **D — Send Email** | `/email/send` endpoint; `ComposeModal.tsx`; Send Email button on contact profiles | ✅ Done |
| **E — Templates** | Template CRUD endpoints; template section in Settings; template picker in compose modal | ✅ Done |
| **F — Open Tracking** | `tracking.ts` route; pixel embed in send; open badge on timeline | ✅ Done |
| **G — Inbox Sync** | `emailSync.ts` service; `startSyncPoller()` in `index.ts`; Synced badge on timeline | ✅ Done |

All phases implemented and pushed to `main`.
...
**Three environments require separate redirect URIs.** All of the following must be registered in Google Cloud Console as authorised redirect URIs — only the one matching the current `GOOGLE_REDIRECT_URI` env var will be used at runtime:
- `http://localhost/api/gmail/callback` — local Docker (`docker compose up`)
- `http://localhost:5173/api/gmail/callback` — local Vite dev server (`npm run dev`)
- `https://crm.yourcompany.com/api/gmail/callback` — production

Using a URI not registered in Google Cloud Console causes a `redirect_uri_mismatch` error on the consent screen.
...
| `backend/Dockerfile` | Builder stage: `npm install`; production stage: copies lockfile from builder then `npm ci --omit=dev` |
| `docker-compose.yml` | Add 5 env vars to `crm-backend` (including `GMAIL_SYNC_INTERVAL_MS`) |
| `.env.example` | Document new env vars with per-environment examples |
...
Testing the Gmail integration with `docker compose up` on a local machine requires a few steps beyond the standard setup.

### Prerequisites

1. A Google Cloud project with the Gmail API enabled
2. An OAuth 2.0 Client ID (Web application type) with `http://localhost/api/gmail/callback` added as an authorised redirect URI

### `.env` values for local Docker

```env
GOOGLE_CLIENT_ID=your_client_id.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=your_client_secret
GOOGLE_REDIRECT_URI=http://localhost/api/gmail/callback
...
Open `http://localhost` in a browser. The **Settings** page (sidebar) has the Gmail connection panel.

### Test checklist

- [ ] Settings → Connect Gmail → Google consent screen appears
- [ ] After authorising, Settings shows green "Connected" badge with Gmail address
- [ ] Open any contact with an email address → **Send Email** button appears in header
- [ ] Compose and send a test email → confirm receipt in the inbox
- [ ] Reload the contact's Timeline → sent email appears as an interaction
- [ ] Create a template in Settings → select it in the compose modal → placeholders substitute correctly
- [ ] Wait 5 minutes after receiving an email from a known contact → check their timeline for the synced entry

### Known local limitations

| Feature | Local Docker | Production |
|---|---|---|
| Send email | ✅ Works | ✅ Works |
| Timeline logging | ✅ Works | ✅ Works |
| Email templates | ✅ Works | ✅ Works |
| Inbox sync | ✅ Works | ✅ Works |
| Open tracking | ❌ Gmail cannot reach `http://localhost` | ✅ Works via Cloudflare tunnel |
